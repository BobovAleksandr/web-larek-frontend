...
2. `currentProduct: IProduct | null` - поле для хранения текущего товара, выбранного пользователем.
3. `selectedProduct(id: string)` - сеттер, принимает id товара, ищет его в массиве товаров и присваивает в поле `currentProduct`
4. `getSelectedProduct(): IProduct` - метод возвращает объект текущего выбранного товара.
...
# Слой презентера
Код взаимодействия слоев предсталвения и данных находится в файле `index.ts`. Презентер предсталяет собой экземпляр класса `EventEmitter`, методы которого позволяют установить обработчик на кастомное событие, вызываемое, снять обработчик , инициировать событие с данными.
